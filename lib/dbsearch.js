...
search.tryToTokenize = function(content) {
	if (!content) {
		return content;
	}
	return tokenizer.run(content);
}
...
search.filterSearchQuery = async function (data) {
	...
	if (data.hasOwnProperty('content')) {
		query.content = search.tryToTokenize(data.content);
	}
...
async function topicsSave(topics) {
	...
	let data = topics.map(function (topicData) {
		const indexData = {};
		if (topicData.title) {
			indexData.content = search.tryToTokenize(topicData.title);
		}
		if (topicData.cid) {
...
async function postsSave(posts) {
	...
	let data = posts.map(function (postData) {
		const indexData = {};
		if (postData.content) {
			indexData.content = search.tryToTokenize(postData.content.replace(/<[^>]*>?/gm, ''));
		}
		if (postData.cid) {
